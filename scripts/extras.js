Quintus.ExtrasScript = function(Q) {
	...
	Q.component("extra", {
		added: function() {
			...
			//si lo tocan al extra por cualquier lado
	...
	//extra que no se destruye (muelle y cambio de direccion)
	Q.component("extraFix", {
		added: function() {
			var entity = this.entity;
			//si lo tocan al extra que no se destruye por cualquier lado
			entity.on("hit",function(collision) {
				if(collision.obj.isA("Player")&&!this.p.hasCollide) {
					//console.log(this.p);
					...
					if (!Q.state.get("mute")){
						if ( this.p.status == "muelle" ){
							Q.audio.play('supersalto.mp3',{ debounce: 100 });
						}else if ( this.p.status == "changeDirection" ){
							Q.audio.play('changeDirection.mp3',{ debounce: 100 });
						}
					}
				}
			});
